...
func (t *Transactor) SetRPCClient(rpcClient *rpc.Client) {
	t.upstream = newRPCTransactor(rpcClient)
}
...
func (t *Transactor) validateAndPropagate(selectedAccount *account.SelectedExtKey, args SendTxArgs) (hash gethcommon.Hash, err error) {
	...
	defer cancel()
	nonce, err = t.upstream.PendingNonceAt(ctx, args.From)
	if err != nil {
		return hash, err
	}
	...
	if args.GasPrice == nil {
		ctx, cancel = context.WithTimeout(context.Background(), t.rpcCallTimeout)
		defer cancel()
		gasPrice, err = t.upstream.SuggestGasPrice(ctx)
		if err != nil {
			return hash, err
		}
	}
	...
	if args.Gas == nil {
		ctx, cancel = context.WithTimeout(context.Background(), t.rpcCallTimeout)
		defer cancel()
		gas, err = t.upstream.EstimateGas(ctx, ethereum.CallMsg{
			From:     args.From,
			To:       args.To,
			GasPrice: gasPrice,
			Value:    value,
	...
	ctx, cancel = context.WithTimeout(context.Background(), t.rpcCallTimeout)
	defer cancel()
	if err := t.upstream.SendTransaction(ctx, signedTx); err != nil {
		return hash, err
	}
